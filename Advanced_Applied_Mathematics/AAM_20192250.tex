...
\begin{document}
	...
	\begin{tcolorbox}[colframe=thmcolor,title={\color{white}\bf Basis Change}]
		\begin{theorem}
			...
			\] of \(W\), and a transformation matrix \(\textbf{A}_{\Phi}\) w.r.t. \(\mathscr{B}\) and \(\mathscr{C}\), the corresponding transformation matrix \(\tilde{\textbf{A}}_\Phi\) w.r.t. the bases \(\tilde{\mathscr{B}}\) and \(\tilde{\mathscr{C}}\) is given \[
			\tilde{\textbf{A}}_\Phi=\textbf{T}^{-1}\textbf{A}_\Phi \textbf{S}.
			\]
		\end{theorem}
	\end{tcolorbox}
	\begin{proof}
		We define \(\textbf{S}=(s_{ij})\)We can write the vectors of the new basis \(\tilde{\mathscr{B}}\) of \(V\) as follow: \[
		\tilde{\textbf{b}}_j=\begin{bmatrix}
			s_{1j} \\ \vdots \\ s_{nj}
		\end{bmatrix}_{\mathscr{B}}=\sum_{i=1}^ns_{ij}\textbf{b}_j\quad\text{for}\quad j=1,\dots,n.
		\] Similarly, we write the new basis vectors \(\tilde{\mathscr{C}}\) of \(W\) as follow: \[
		\tilde{\textbf{c}}_j=\begin{bmatrix}
			t_{1k} \\ \vdots \\ t_{mk}
		\end{bmatrix}_{\mathscr{C}}=\sum_{l=1}^mt_{lk}\textbf{c}_l\quad\text{for}\quad k=1,\dots,m.
		\]
	\end{proof}
	
	\begin{center}
	\begin{tikzcd}
		\text{Vector spaces} &  &  & V \arrow[rr, "\Phi"]                                             &  & W                                    &  &  & V \arrow[rr, "\Phi"]                                             &  & W                                \\
		&  &  & \mathscr{B} \arrow[rr, "\textbf{A}_\Phi"]                                 &  & \mathscr{C}                          &  &  & \mathscr{B} \arrow[rr, "\textbf{A}_\Phi"]                                 &  & \mathscr{C} \arrow[dd, "\textbf{T}^{-1}"] \\
		\text{Ordered bases} &  &  &                                                                  &  &                                      &  &  &                                                                  &  &                                  \\
		&  &  & \tilde{\mathscr{B}} \arrow[uu, "\textbf{S}"] \arrow[rr, "\tilde{A}_\Phi"] &  & \tilde{\mathscr{C}} \arrow[uu, "\textbf{T}"'] &  &  & \tilde{\mathscr{B}} \arrow[uu, "\textbf{S}"] \arrow[rr, "\tilde{A}_\Phi"] &  & \tilde{\mathscr{C}}             
	\end{tikzcd}
	\end{center}
	
	\begin{example}
		Consider the linear mapping $T:\R^2\to\R^2$ whose transformation is \[
		A_T=
